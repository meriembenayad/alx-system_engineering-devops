...
<details>
<summary>0. Sky is the limit, let's bring that limit higher</summary>

n this web stack debugging task, we are evaluating the performance of our web server setup featuring Nginx under stress conditions. Unfortunately, the results indicate poor performance, with a substantial number of failed requests.

For the benchmarking, we utilize ApacheBench, a widely-used tool in the industry. This tool allows us to simulate HTTP requests to a web server. In this instance, I conducted 2000 requests to my server, with 100 requests occurring simultaneously. The analysis revealed that 943 requests failed. Our goal is to rectify our stack issues and reduce this number to zero. It's crucial to remember that when encountering issues, logs play a crucial role in troubleshooting.
...
<details>
<summary>1. User limit</summary>

Modify the operating system configuration to enable logging in with the "holberton" user and opening a file without encountering any error messages.
